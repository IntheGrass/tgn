...
class AttnScoreLayer(torch.nn.Module):
    ...
    def forward(self, Q, K):
        ...
        attention_scores = torch.sum(Q_proj * K_proj, dim=1, keepdim=True)  # (n,1)
...
def get_in_neighbor_finder(data, uniform, max_node_idx=None):
    # 区分边方向，仅采样入邻居节点的采样器
    # 例如：对于 src -> dst的边，dst节点视为邻居包含src节点，而src节点的邻居不包含dst节点
    max_node_idx = max(data.sources.max(), data.destinations.max()) if max_node_idx is None else max_node_idx
    adj_list = [[] for _ in range(max_node_idx + 1)]
    for source, destination, edge_idx, timestamp in zip(data.sources, data.destinations,
                                                        data.edge_idxs,
...
def get_out_neighbor_finder(data, uniform, max_node_idx=None):
    # 区分边方向，仅采样出邻居节点的采样器
    # 例如：对于 src -> dst的边，src节点视为邻居包含dst节点，而dst节点的邻居不包含src节点
    max_node_idx = max(data.sources.max(), data.destinations.max()) if max_node_idx is None else max_node_idx
    adj_list = [[] for _ in range(max_node_idx + 1)]
    for source, destination, edge_idx, timestamp in zip(data.sources, data.destinations,
                                                        data.edge_idxs,
