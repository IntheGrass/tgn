    ...
    with torch.no_grad():
        ...
        for i, test_node in tqdm(enumerate(test_keys), total=len(test_keys), desc="eval model"):
            ...
            for k in range(batch_num):
                ...
                pred_score = model.predict_edge_probabilities(source_nodes, destination_nodes, timestamps,
...
def main():
    ...
    tgn.device = device
    tgn.graph_embedding_layer.device = device
    tgn.graph_embedding_layer.embedding_module.device = device
    tgn.to(device)
